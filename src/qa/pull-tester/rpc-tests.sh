...
# Get BUILDDIR and REAL_ENTANGLEMENTD
. "${CURDIR}/tests-config.sh"

export ENTANGLEMENTCLI=${BUILDDIR}/qa/pull-tester/run-entanglement-cli
export ENTANGLEMENTD=${REAL_ENTANGLEMENTD}

#Run the tests

if [ "x${ENABLE_ENTANGLEMENTD}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and entanglementd must all be enabled"
fi
