...
/*
 * Simple DNS server for Entanglement project
 *
...
SeqDns::SeqDns(const char *bind_ip, uint16_t port_no,
   const char *gw_suffix, const char *allowed_suff, const char *local_fname, const char *enums, const char *tollfree, uint8_t verbose) 
    ...
    m_dap_ht  = (allowed_len && m_gw_suf_len)? (DNSAP*)calloc(ETGDNS_DAPSIZE, sizeof(DNSAP)) : NULL; 
...
DNSAP *SeqDns::CheckDAP(uint32_t ip_addr) { 
  ...
  hash ^= hash >> 16;
  hash += hash >> 8;
  DNSAP *dap = m_dap_ht + (hash & (ETGDNS_DAPSIZE - 1));
  ...
  return (dap->ed_size <= ETGDNS_DAPTRESHOLD)? dap : NULL;
} // SeqDns::CheckDAP 
...
bool SeqDns::CheckEnumSig(const char *q_str, char *sig_str) {
  ...
  CEntanglementAddress addr(nti.strAddress);
        if(!addr.IsValid())
