...
CBlockTemplate* CreateNewBlockInner(const CScript& scriptPubKeyIn, bool fAddProofOfStake, bool& fPoSCancel, CWallet* pwallet)
{
    ...
    if (fAddProofOfStake)  // attemp to find a coinstake
    {
        ...
        if (fPoSCancel)
            return NULL; // Entanglement: there is no point to continue if we failed to create coinstake
    }
    else
...

    {
        ...
        CValidationState state;
        if (!TestBlockValidity(state, *pblock, pindexPrev, false, false, false)) // Entanglement: we do not check block signature here, since we did not sign it yet
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("EntanglementMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, Params(), NULL, pblock))
        return error("EntanglementMiner : ProcessNewBlock, block not accepted");

    return true;
}

void EntanglementMiner(CWallet *pwallet, bool fProofOfStake)
{
    LogPrintf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread(fProofOfStake? "entanglement-stake-minter" : "entanglement-miner");
    ...
    try {
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in EntanglementMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("Running EntanglementMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("EntanglementMiner terminated\n");
	return;
        // throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("EntanglementMiner runtime error: %s\n", e.what());
        return;
    }
}

void GenerateEntanglements(bool fGenerate, CWallet* pwallet, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EntanglementMiner, pwallet, false));
}
...
void static ThreadStakeMinter(void* parg)
{
    ...
    try
    {
        EntanglementMiner(pwallet, true);
    }
