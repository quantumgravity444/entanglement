...
#ifndef ENTANGLEMENT_SERIALIZE_H
#define ENTANGLEMENT_SERIALIZE_H
...
enum
{
    ...
    // modifiers
    SER_POSMARKER       = (1 << 18),  // Entanglement: for sending block headers with PoS marker, to allow headers-first syncronization
};
 ...
 * Thus, the byte entanglement a[] with length len, where all but the last byte
 ...
 * * No redundancy: every (infinite) byte entanglement corresponds to a list
 *   of encoded integers.
 * 
...
#endif // ENTANGLEMENT_SERIALIZE_H
