...
const QString EntanglementGUI::DEFAULT_WALLET = "~Default";

EntanglementGUI::EntanglementGUI(const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    ...
    spinnerFrame(0)
{
    ...
    QString windowTitle = tr("Entanglement") + " - ";
#ifdef ENABLE_WALLET
...
EntanglementGUI::~EntanglementGUI()
{
...
void EntanglementGUI::createActions(const NetworkStyle *networkStyle)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Entanglement address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and Entanglement: URIs)"));
    ...
    dnsAction->setStatusTip(tr("Manage values registered via Entanglement"));
    ...
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About Entanglement"), this);
    aboutAction->setStatusTip(tr("Show information about Entanglement"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Entanglement"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Entanglement addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Entanglement addresses"));
    ...
    openAction->setStatusTip(tr("Open a Entanglement: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Entanglement help message to get a list with possible Entanglement command-line options"));
...
void EntanglementGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void EntanglementGUI::createToolBars()
{
    if(walletFrame)
    {
...
void EntanglementGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool EntanglementGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool EntanglementGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void EntanglementGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void EntanglementGUI::setWalletActionsEnabled(bool enabled)
{
...
void EntanglementGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("Entanglement client") + " " + networkStyle->getTitleAddText();
...
void EntanglementGUI::createTrayIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void EntanglementGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void EntanglementGUI::optionsClicked()
{
...
void EntanglementGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void EntanglementGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}


void EntanglementGUI::showBackups()
{
    GUIUtil::showBackups();
}

void EntanglementGUI::showHelpMessageClicked()
{
...
#ifdef ENABLE_WALLET
void EntanglementGUI::openClicked()
{
...
void EntanglementGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void EntanglementGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void EntanglementGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void EntanglementGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void EntanglementGUI::gotoMultiSigPage()
{
    multiSigAction->setChecked(true);
    if (walletFrame) walletFrame->gotoMultiSigPage();
}

void EntanglementGUI::gotoStakeReportPage()
{
    stakeReportAction->setChecked(true);
    if (walletFrame) walletFrame->gotoStakeReportPage();
}

void EntanglementGUI::gotoDNSPage()
{
    dnsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoDNSPage();
}

void EntanglementGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void EntanglementGUI::gotoVerifyMessageTab(QString addr)
{
...
void EntanglementGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Entanglement network", "", count));
}

void EntanglementGUI::updateHeadersSyncProgressLabel()
{
...
void EntanglementGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void EntanglementGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Entanglement"); // default title
...
void EntanglementGUI::changeEvent(QEvent *e)
{
...
void EntanglementGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
#ifdef ENABLE_WALLET
void EntanglementGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
    ...
    message((amount)<0 ? tr("Sent transaction") : tr("Incoming transaction"),
             tr("Date: %1\n"
                ...
                "Address: %4\n")
                  .arg(date)
                  .arg(EntanglementUnits::formatWithUnit(unit, amount, true))
                  .arg(type)
                  .arg(address), CClientUIInterface::MSG_INFORMATION);
}
#endif // ENABLE_WALLET

void EntanglementGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void EntanglementGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool EntanglementGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool EntanglementGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void EntanglementGUI::setHDStatus(int hdEnabled)
{
...
void EntanglementGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void EntanglementGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void EntanglementGUI::toggleHidden()
{
...
void EntanglementGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void EntanglementGUI::updateStakingIcon()
{
    updateWeight();
...
void EntanglementGUI::detectShutdown()
{
...
void EntanglementGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void EntanglementGUI::showModalOverlay()
{
    if (modalOverlay)
        modalOverlay->showHide(false, true);
}

static bool ThreadSafeMessageBox(EntanglementGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void EntanglementGUI::subscribeToCoreSignals()
{
    // Connect signals to client
    uiInterface.ThreadSafeMessageBox.connect(boost::bind(ThreadSafeMessageBox, this, _1, _2, _3));
}

void EntanglementGUI::unsubscribeFromCoreSignals()
{
...
void EntanglementGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<EntanglementUnits::Unit> units = EntanglementUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const EntanglementUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(EntanglementUnits::name(unit)));
    }
...
{
    menu = new QMenu();
    Q_FOREACH(EntanglementUnits::Unit u, EntanglementUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(EntanglementUnits::name(u)), this);
...
{
    setPixmap(QIcon(":/icons/unit_" + EntanglementUnits::id(newUnits)).pixmap(33,STATUSBAR_ICONSIZE));
}
