...
{
    CEntanglementAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, int nSplitBlock, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered Entanglement address / amount:
            ...
            CScript scriptPubKey = GetScriptForDestination(CEntanglementAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        // reject insane fee > 0.1 Entanglement
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal Entanglement:URI (Entanglement:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CEntanglementAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    for(const COutput& out : vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CEntanglementAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CEntanglementAddress(sAddress).Get();
...
bool WalletModel::isMine(const CEntanglementAddress &address)
{
