...
void MultisigDialog::on_createAddressButton_clicked()
{
    ...
    {
        QMessageBox::critical(this, tr("Multisig: Chain Download Incomplete!"), tr("Entanglement is downloading blocks..."));
        return;
    }
    ...
    for(unsigned int i = 0; i < countPublicKeyEntered; i++)
    {
        ...
        {
            CEntanglementAddress address(strAddressEntered);
    ...
    if (unknownAddressCount == 0)
    {
        ...
        CEntanglementAddress multiSigAddress(scriptID);
    ...
    else
    {
        ...
        {
            // TODO (Amir): If user agrees to advertise their public key using DDNS as a multisig name (fee = 0.001 ETG)
...
void MultisigDialog::on_saveMultisigAddressButton_clicked()
{
    ...
    CEntanglementAddress entanglementMultiSigAddress(address);
    if(!pwalletMain->mapAddressBook.count(entanglementMultiSigAddress.Get()))
        pwalletMain->SetAddressBook(entanglementMultiSigAddress.Get(), label, "multisig address");
...
void MultisigDialog::on_transaction_textChanged()
{
    ...
    for(const CTxOut& txout : tx.vout)
    {
        ...
        CEntanglementAddress address(addr);
...
void MultisigDialog::on_sendTransactionButton_clicked()
{
    ...
    if(fee < minFee)
    {
        QMessageBox::StandardButton ret = QMessageBox::question(this, tr("Confirm send transaction"), tr("The fee of the transaction (%1 ETG) is smaller than the expected fee (%2 ETG). Do you want to send the transaction anyway?").arg((double) fee / COIN).arg((double) minFee / COIN), QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel);
        if(ret != QMessageBox::Yes)
            return;
    }
    else if(fee > minFee)
    {
        QMessageBox::StandardButton ret = QMessageBox::question(this, tr("Confirm send transaction"), tr("The fee of the transaction (%1 ETG) is bigger than the expected fee (%2 ETG). Do you want to send the transaction anyway?").arg((double) fee / COIN).arg((double) minFee / COIN), QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel);
        if(ret != QMessageBox::Yes)
            return;
    }
