...
void SendCoinsDialog::setModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        // Entanglement: disable fee section
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    {
        SendCoinsEntry *entry = qobject_cast<SendCoinsEntry*>(ui->entries->itemAt(i)->widget());
        CEntanglementAddress address = entry->getValue().address.toStdString();
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        if (!model->getSplitBlock())
        {
            ...
            QString amount = "<b>" + EntanglementUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount / nSplitBlock);
            amount.append("</b>");
    ...
    processSendCoinsReturn(prepareStatus,
        EntanglementUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), currentTransaction.getTransactionFee()));
    ...
    if(txFee > 0)
    {
        ...
        questionString.append(EntanglementUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    foreach(EntanglementUnits::Unit u, EntanglementUnits::availableUnits())
    {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(EntanglementUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(tr("Total Amount %1 (= %2)")
        .arg(EntanglementUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount))
                                 ...
                                 const CAmount& watchBalance, const CAmount& watchStake, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
    ...
    if(model && model->getOptionsModel())
    {
        ui->labelBalance->setText(EntanglementUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance));
    }
}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::InsaneFee:
        msgParams.first = tr("A fee higher than %1 is considered an insanely high fee.").arg(EntanglementUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), 10000000));
        break;
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(EntanglementUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) +
...
void SendCoinsDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(tr("Pay only the minimum fee of %1").arg(
            EntanglementUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), CWallet::minTxFee.GetFeePerK()) + "/kB")
        );
}
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    {
        ui->labelSmartFee->setText(EntanglementUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), CWallet::minTxFee.GetFeePerK()) + "/kB");
        ui->labelSmartFee2->show(); // (Smart fee not initialized yet. This usually takes a few blocks...)
        ui->labelFeeEstimation->setText("");
    }
    else
    {
        ui->labelSmartFee->setText(EntanglementUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), feeRate.GetFeePerK()) + "/kB");
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CEntanglementAddress addr = CEntanglementAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Entanglement address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
