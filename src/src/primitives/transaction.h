...
#ifndef ENTANGLEMENT_PRIMITIVES_TRANSACTION_H
#define ENTANGLEMENT_PRIMITIVES_TRANSACTION_H
...
public:
    COutPoint prevout;
    CScript scriptSig;
    uint32_t nEntanglement;

    CTxIn()
    {
        nEntanglement = std::numeric_limits<unsigned int>::max();
    }
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        READWRITE(prevout);
        READWRITE(scriptSig);
        READWRITE(nEntanglement);
    }

    bool IsFinal() const
    {
        return (nEntanglement == std::numeric_limits<uint32_t>::max());
    }
    ...
    friend bool operator==(const CTxIn& a, const CTxIn& b)
    {
        return (a.prevout   == b.prevout &&
                ...
                a.nEntanglement == b.nEntanglement);
    }
...
#endif // ENTANGLEMENT_PRIMITIVES_TRANSACTION_H
