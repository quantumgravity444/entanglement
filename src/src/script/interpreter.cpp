...
public:
    ...
    void SerializeInput(S &s, unsigned int nInput, int nType, int nVersion) const {
        ...
        // Serialize the nEntanglement
        if (nInput != nIn && (fHashSingle || fHashNone))
            // let the others update at will
            ::Serialize(s, (int)0, nType, nVersion);
        else
            ::Serialize(s, txTo.vin[nInput].nEntanglement, nType, nVersion);
    }
...
bool TransactionSignatureChecker::CheckLockTime(const CScriptNum& nLockTime) const
{
    ...
    // finalized by setting nEntanglement to maxint. The
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror, bool fNamecoin)
{
    ...
    // Entanglement: for backward compatability namecoin script should not be checked for minimaldata
    if (fNamecoin)
...
bool AddressMatchesPubKey(const CNameVal& name, const CNameVal& value, std::string& strError)
{
    ...
    if (CEntanglementAddress(keyID).ToString() != strAddress)
    {
        strError = CEntanglementAddress(keyID).ToString() + " != Base58(SHA256(" + strAddress + "))";
        return false;
    }
    return true;
}
