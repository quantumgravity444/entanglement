...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "It is set with the command line argument -gen (or entanglement.conf setting gen)\n"
...
UniValue setgenerate(const UniValue& params, bool fHelp)
{
    ...
    else // Not -regtest: start generate thread, return immediately
    {
        ...
        GenerateEntanglements(fGenerate, pwalletMain, nGenProcLimit, Params());
    }
...
// NOTE: Unlike wallet RPC (which use ENTANGLEMENT values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getwork(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Entanglement is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Entanglement is downloading blocks...");
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "See https://en.entanglement.it/wiki/BIP_0022 for full specification.\n"
