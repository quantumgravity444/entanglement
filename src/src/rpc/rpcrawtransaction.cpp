...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, UniValue& entry)
{
    ...
    for(const CTxIn& txin : tx.vin) {
        ...
        in.push_back(Pair("entanglement", (int64_t)txin.nEntanglement));
        vin.push_back(in);
    }
...
UniValue getrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "         \"hex\": \"hex\"   (string) hex\n"
            "       },\n"
            "       \"entanglement\": n      (numeric) The script entanglement number\n"
            "     }\n"
            "     ,...\n"
            "  ],\n"
            "  \"vout\" : [              (array of json objects)\n"
            "     {\n"
            "       \"value\" : x.xxx,            (numeric) The value in Entanglement\n"
            "       \"n\" : n,                    (numeric) index\n"
            ...
            "           \"entanglementaddress\"           (string) Entanglement address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric or std::string, required) The key is the Entanglement address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
    ...
    for (unsigned int idx = 0; idx < inputs.size(); idx++) {
        ...
        uint32_t nEntanglement = (rawTx.nLockTime ? std::numeric_limits<uint32_t>::max() - 1 : std::numeric_limits<uint32_t>::max());
        CTxIn in(COutPoint(txid, nOutput), CScript(), nEntanglement);
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) Entanglement address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue decodescript(const UniValue& params, bool fHelp)
{
    ...
    if (type.isStr() && type.get_str() != "scripthash") {
        ...
        r.push_back(Pair("p2sh", CEntanglementAddress(CScriptID(script)).ToString()));
    }
    return r;
}
...
static void TxInErrorToJSON(const CTxIn& txin, UniValue& vErrorsRet, const std::string& strMessage)
{
    ...
    entry.push_back(Pair("entanglement", (uint64_t)txin.nEntanglement));
