...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake, const Consensus::Params& params)
{
    ...
    if (nActualSpacing < 0) {
        nActualSpacing = nPoWTargetSpacing;
    }
    else if (fProofOfStake && nActualSpacing > nPoWTargetSpacing * 10) {
         nActualSpacing = nPoWTargetSpacing * 10;
    }
    ...
    bnNew *= ((nInterval - 1) * nPoWTargetSpacing + nActualSpacing + nActualSpacing);
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, const Consensus::Params& params)
{
    ...
    // Check range
    if (fNegative || bnTarget == 0 || fOverflow || bnTarget > consensusParams.bnProofOfWorkLimit)
