...
#if defined(NDEBUG)
# error "Entanglement cannot be compiled without assertions."
#endif
...
CHooks* hooks = InitHook(); //this adds namecoin hooks which allow splicing of code inside standart entanglement functions.

/** Constant stuff for coinbase transactions we create: */
CScript COINBASE_FLAGS;

const string strMessageMagic = "Entanglement Signed Message:\n";
...
CAmount GetProofOfStakeReward()
{
    ...
    if(chainActive.Height() <= 101325) 
    {
        CAmount nSubsidy = 69.3 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean, bool fWriteNames)
{
    ...
    // Entanglement: undo name transactions in reverse order
    if (fWriteNames)
        for (int i = block.vtx.size() - 1; i >= 0; i--)
...
void ThreadScriptCheck() {
    RenameThread("entanglement-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fWriteNames)
{
    ...
    // Entanglement: collect valid name tx
    ...
    // Entanglement: add names to nameindex.dat
    if (fWriteNames)
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    assert(pindexNew);
    // We assign the entanglement id to blocks only when the full data is available,
...
bool AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    // Entanglement: set PoS flag for CBlockIndex. This should probably be done immediately after we added header to CBlockIndex (i.e., after or inside AddToBlockIndex),
...
{
    // Preliminary checks
    //bool checked = CheckBlock(*pblock, state); // Entanglement: removed, since this check happens later in AcceptBlock function
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    // Entanglement: set/unset network serialization mode for new clients
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        for(const CBlockHeader& header : headers) {
            ...
            if (pindexLast != NULL && header.hashPrevBlock != pindexLast->GetBlockHash()) {
                Misbehaving(pfrom->GetId(), 20);
                return error("non-continuous headers entanglement");
            }
