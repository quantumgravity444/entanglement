...
#else
    const char* pszModule = "entanglement";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\entanglement
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\entanglement
    // Mac: ~/Library/Application Support/entanglement
    // Unix: ~/.entanglement
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Entanglement";
#else
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "entanglement.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create entanglement.conf if it does not exist
        FILE* configFile = fopen(GetConfigFile().string().c_str(), "a");
        if (configFile != NULL) {
            // Write entanglement.conf file with random username and password.
            WriteConfigFile(configFile);
            // New entanglement.conf file written, now read it.
        }
    }
    ...
    {
        // Don't overwrite existing settings so command line settings override entanglement.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "entanglementd.pid"));
