...
BOOST_AUTO_TEST_CASE(MempoolRemoveTest)
{
    ...
    // Just the parent:
    testPool.addUnchecked(txParent.GetHash(), CTxMemPoolEntry(txParent, 0, 0, 0.0, 1), true);
    testPool.remove(txParent, removed, true);
    ...
    for (int i = 0; i < 3; i++)
    {
        testPool.addUnchecked(txChild[i].GetHash(), CTxMemPoolEntry(txChild[i], 0, 0, 0.0, 1), true);
        testPool.addUnchecked(txGrandChild[i].GetHash(), CTxMemPoolEntry(txGrandChild[i], 0, 0, 0.0, 1), true);
    }
...
BOOST_AUTO_TEST_CASE(MempoolIndexingTest)
{
    ...
    std::string dummy;
    CTxMemPoolEntry entry7(tx7, 2000000LL, 1, 10.0, 1);
    ...
    pool.addUnchecked(tx7.GetHash(), CTxMemPoolEntry(tx7, 2000000LL, 1, 10.0, 1), setAncestors);
    ...
    pool.addUnchecked(tx8.GetHash(), CTxMemPoolEntry(tx8, 0LL, 2, 10.0, 1), setAncestors);
    ...
    pool.addUnchecked(tx9.GetHash(), CTxMemPoolEntry(tx9, 0LL, 3, 10.0, 1), setAncestors);
    ...
    CTxMemPoolEntry entry10(tx10, 200000LL, 4, 10.0, 1);
    BOOST_CHECK_EQUAL(pool.CalculateMemPoolAncestors(entry10, setAncestorsCalculated, 100, 1000000, 1000, 1000000, dummy), true);
    BOOST_CHECK(setAncestorsCalculated == setAncestors);

    pool.addUnchecked(tx10.GetHash(), CTxMemPoolEntry(tx10, 200000LL, 4, 10.0, 1), setAncestors);
