...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CEntanglementAddress(CScriptID(redeemScript)).ToString();
    ...
    for (int nRep = 0; nRep < iterations && nBest != nTargetValue; nRep++)
    {
        ...
        for (int nPass = 0; nPass < 2 && !fReachedTarget; nPass++)
        {
            for (unsigned int i = 0; i < vValue.size(); i++)
            {
                ...
                //that the rng is fast. We do not use a constant random entanglement,
                                ...
                                CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int nSplitBlock, std::string& strFailReason, const CCoinControl* coinControl)
{
    ...
    // Entanglement: define some values used in case of namecoin tx creation
    ...
    txNew.nVersion = wtxNew.nVersion; // Entanglement: important for name transactions

    {
        LOCK2(cs_main, cs_wallet);
        {
            nFeeRet = max(nFeeInput, MIN_TX_FEE);  // Entanglement: a good starting point, probably...
            while (true)
            {
                txNew.vin.clear();
                txNew.vout.clear();
                ...
                // Entanglement: in case of namecoin tx we have already supplied input.
                ...
                // otherwise proceed as we normaly would in entanglement
                else
        ...
        // Entanglement: name tx always at first position
                if (!wtxNameIn.IsNull())
                {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-entanglement-address
                ...
                for(const std::pair<const CWalletTx*, unsigned int>& coin : setCoins)
                {
                    // Entanglement: we sign name tx differently.
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CEntanglementAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CEntanglementAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CEntanglementAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CEntanglementAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CEntanglementAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CEntanglementAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CEntanglementAddress(dest).ToString(), key);
}
